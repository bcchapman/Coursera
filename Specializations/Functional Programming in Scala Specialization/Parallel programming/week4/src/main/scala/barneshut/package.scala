...
package object barneshut {
  ...
  case class Fork(
    ...
    val centerX: Float = (nw.centerX + ne.centerX + sw.centerX + se.centerX) / 4 // average center x
    val centerY: Float = (nw.centerY + ne.centerY + sw.centerY + se.centerY) / 4 // average center y
    val size: Float = nw.size + ne.size // size is sum of north nodes
    val mass: Float = nw.mass + ne.mass + sw.mass + se.mass // sum of all nodes mass

    // these two are derived from notes in example, as well as divide by zero safety
    ...
    val total: Int = nw.total + ne.total + sw.total + se.total // sum total of all
