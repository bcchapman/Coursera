...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    var empty = GenMap[Point, GenSeq[Point]]()
    means.foreach(mean => empty += (mean -> List[Point]()))

    //merge with empty
    empty ++ points.par.groupBy(findClosest(_, means))
  }
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    // check if square distance is <= eta for ALL pairs
