...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    input.zipWithIndex
      .foreach {
        case (x, 0) => output(0) = 0 // prevent division by zero
        case (x, i) => output(i) = Math.max(x / i, output(i-1)) // max of current val or previous
      }
  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    //bound check
    if(until - from <= 0) return

    //determine next max angle, update and continue
    val maxAngle = Math.max(startingAngle, input(from) / from)
    output.update(from, maxAngle)
