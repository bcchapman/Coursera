...
class StackOverflow extends Serializable {
  ...
  def scoredPostings(grouped: RDD[(Int, Iterable[(Posting, Posting)])]): RDD[(Posting, Int)] = {
    ...
    grouped
      .flatMap(x => x._2) // expand pair values
      .groupByKey() // grouped by posting id
      .mapValues(v => answerHighScore(v.toArray)) // and extract max value
  }
  ...
  def vectorPostings(scored: RDD[(Posting, Int)]): RDD[(Int, Int)] = {
    ...
    (
      for(score <- scored if firstLangInTag(score._1.tags, langs).isDefined)
      yield
        (langSpread * firstLangInTag(score._1.tags, langs).get, score._2)
    ).persist() // persist here is key. This is used multiple times
  }
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] = {
    ...
    val clusters = cachedVectors.map(v => (findClosest(v, means), v)).groupByKey()
    val updatedClusters = clusters.mapValues(averageVectors).collect
