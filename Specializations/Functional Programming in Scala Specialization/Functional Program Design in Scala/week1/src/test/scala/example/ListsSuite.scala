 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of a few numbers with a negative") {
    assert(sum(List(1, 2, -5)) === -2)
  }
  test("sum of a few numbers with repeats") {
    assert(sum(List(1, 2, 2, 0)) === 5)
  }
  ...
  test("max of a few numbers with a negative") {
    assert(max(List(1, 2, -5)) === 2)
  }
  test("max of a few numbers with repeats") {
